# LAB1-02 전가산기 (반가산기 두 개의 연결) — 실험 전 레포트

- 과목: 전자전기컴퓨터설계실험Ⅱ / LAB1 조합논리 (교육 번호 02, 기존 번호 3)
- 작성자: 엄상혁 (학번 2025440084) / 조: ______ / 작성일: 2026-09-__
- 설계 모듈: `full_adder` / TB: `tb_full_adder` / 저장소: https://github.com/dhawldnjs010-star/lab1_02_full_adder
- 소스 커밋: `c2c88f3` (`c2c88f380d5622390cde18cf3393613125fd0816`)

## 1. 실험 목적과 확인할 것

입력 조합과 출력의 관계를 **진리표·파형으로 설명**한다. 이 실험의 입출력은 입력 `a, b, cin` / 출력 `s`(합), `cout`(올림).
...
작성 파일: `src/half_adder.v`, `src/full_adder.v`, `sim/tb_full_adder.sv`, `constraints/pins.xdc`
...
**설계 설명.** `half_adder`(s=a^b, c=a&b) 두 개를 직렬로 연결했다. 1단은 a+b, 2단은 (a+b)+cin이며 `cout = carry_ab | carry_cin`이다.

| 파일 | 역할 |
|---|---|
| `src/*.v` | 설계(RTL). 합성 대상이며 TB를 넣지 않는다. |
| `sim/tb_full_adder.sv` | DUT 연결, 입력 자극, 예상값 검사(`$fatal`), `wave.vcd` 덤프(`$dumpfile`·`$dumpvars`), `$finish`. 입력을 10 ns 간격으로 바꾸며 8개 조합을 검사한다. |
| `constraints/pins.xdc` | 보드 핀 배정(PACKAGE_PIN·IOSTANDARD·get_ports). Icarus 기능 시뮬레이션의 입력이 아니다. |
| `simulation.json` | `sources`, `testbench`(`sim/tb_full_adder.sv`), `simulation_top`(`tb_full_adder`) 지정. |

## 4. 핀 제약(XDC) 설명과 상태 — **정상**

모든 포트(5개)에 PACKAGE_PIN과 IOSTANDARD(LVCMOS33)가 지정되어 있다.
...
| 항목 | 결과 |
|---|---|
| 콘솔 PASS 문구 | `LAB1_PASS full_adder cases=8` |
...
| 수정 제안 | `assign cout = carry_ab | carry_cin;` → `&` |
| 기대되는 실패 | 올림이 발생하는 행에서 cout 불일치 — 실패 로그에서 vector·expected·actual 확인 |
...
- 확인 계획: a, b, cin 8가지 조합에서 s·cout LED를 진리표와 비교한다(cin=1일 때 올림 전파 포함).
